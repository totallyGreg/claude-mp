...
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core/scripts/*), Bash(osascript:*), Bash(gh issue create:*), Bash(gh issue view:*), Bash(mkdir:*), Bash(cat:*), Read, Write, AskUserQuestion
---
...
```bash
cd "${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core" && osascript -l JavaScript scripts/manage_omnifocus.js bulk-create --json-file /tmp/of-plan-bulk-create.json
```
...
```bash
gh issue create --title "<plan title>" --label "plugin:omnifocus-core" --body "$(cat <<'EOF'
## Summary
...
```bash
"${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core/scripts/ofo" update "<project-id>" --note-append "Issue: <issue-url>"
```
