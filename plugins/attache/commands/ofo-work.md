...
allowed-tools: Bash(${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core/scripts/*), Bash(osascript:*), Bash(mkdir:*), Bash(cat:*), Read, Write, Edit, Glob, Grep, Agent, AskUserQuestion
---
...
```bash
cd "${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core" && osascript -l JavaScript scripts/gtd-queries.js --action ai-agent-tasks
```
...
```bash
cd "${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core" && osascript -l JavaScript scripts/manage_omnifocus.js project-info --name "<selected project name>"
```
...
4. **Mark complete in OmniFocus**:
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core/scripts/ofo complete "<task-id>"
   ```
