...
against the **skill root** (`skills/omnifocus-core/`). Both `gtd-queries.js` and
 ...
 * Run from the skills/omnifocus-core/ root so paths resolve correctly.
...
**Key points:**
- CWD must be `skills/omnifocus-core/` when the script runs — callers are responsible for this
...
cd "$SCRIPTS_DIR"                    # CWD = .../skills/omnifocus-core/scripts/
...
- Shell scripts that invoke JXA scripts must `cd` to the **skill root**
  (`skills/omnifocus-core/`), not into `scripts/`
- Reference JXA scripts with the `scripts/` prefix: `osascript -l JavaScript scripts/gtd-queries.js`
- Commands (`.md` files) reference scripts via `${CLAUDE_PLUGIN_ROOT}/skills/omnifocus-core/scripts/`
